...
{
  // invariant: at each execution of the gaurd, 
  //   A[0...k-1] contains data randomly generated by the rand() function
  for( long k = 0; k < size; k++ )
  {
  ...
  else if( state == reversesorted )
  {
    mergesort( A, 0, size - 1 );
    // invariant: at each execution of the gaurd,
    //   A[0...j-1] and A[size-j...size-1] is reverse sorted
...
void datacopy( const T* const source, T* const destinantion, const long size )
{
  ...
  //   destinantion[0...k-1] == source[0...k-1]
  for( long k = 0; k < size; k++ )
...
// Description: main() runs time complexity tests on the sorting algorithms.
// Precondition: none.
// Postcondition: Time complexity test results are outputted into cout.
int main()
{
  long data[MAX_SIZE];
